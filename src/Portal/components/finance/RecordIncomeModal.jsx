...
const RecordIncomeModal = ({ onClose, onSuccess }) => {
    const [form, setForm] = useState({ incometype: '', incometype_id: '', amount: '', date: new Date().toISOString().split('T')[0], description: '' });
    const [types, setTypes] = useState([]);
    const [submitting, setSubmitting] = useState(false);
    const [error, setError] = useState('');

    useEffect(() => {
        api.get('get_income_types.php').then(d => { if (d.success) setTypes(d.data.types || []); }).catch(() => {});
    }, []);

    const handleTypeChange = (e) => {
        const selectedId = e.target.value;
        const type = types.find(t => String(t.id) === selectedId);
        setForm(f => ({ ...f, incometype_id: selectedId, incometype: type ? type.name : '' }));
    };

    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!form.incometype_id || !form.amount || !form.date) { setError('Please fill required fields'); return; }
        setSubmitting(true);
        setError('');
        try {
            const data = await api.post('add_income.php', { ...form, incometype_id: parseInt(form.incometype_id), amount: parseFloat(form.amount) });
            if (data.success) { onSuccess?.(); onClose(); }
            else setError(data.error || 'Failed to record income');
        } catch (err) { setError('Failed to record income'); }
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(5px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
            <div style={{ background: 'var(--surface-1, #1A1625)', borderRadius: '1rem', padding: '2rem', width: '90%', maxWidth: '500px', border: '1px solid var(--border-color, rgba(255,255,255,0.1))', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                    <h2 style={{ margin: 0, fontSize: '1.3rem', color: 'var(--text-color)' }}>💰 Record Income</h2>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    <div>
                        <label style={{ display: 'block', fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '0.3rem', fontWeight: '600' }}>Income Type *</label>
                        <select value={form.incometype_id} onChange={handleTypeChange}
