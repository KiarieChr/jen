...
const MembersTable = () => {
    ...
    return (
        <div style={{
            ...
            {/* Tabs & Filters Header */}
            <div style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', padding: '1rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
                    ...
                    <div style={{ display: 'flex', background: 'rgba(255,255,255,0.05)', borderRadius: '0.5rem', padding: '0.25rem' }}>
                        <button
                            ...
                            style={{
                                background: activeTab === 'regular' ? '#22c1e6' : 'transparent',
                                color: activeTab === 'regular' ? '#120D20' : '#94a3b8',
                                border: 'none',
                        ...
                        <button
                            ...
                            style={{
                                background: activeTab === 'committed' ? '#22c1e6' : 'transparent',
                                color: activeTab === 'committed' ? '#120D20' : '#94a3b8',
                                border: 'none',
                                padding: '0.5rem 1.5rem',
                                borderRadius: '0.3rem',
                                fontSize: '0.9rem',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', paddingTop: '0.5rem' }}>
                    <span style={{ fontSize: '0.8rem', color: '#64748b', marginRight: '0.5rem' }}>Export:</span>
            ...
            <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
                    ...
                    <tbody>
                        {members.map(member => (
                            <tr key={member.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)', color: '#cbd5e1' }}>
                                <td style={{ padding: '1rem', fontFamily: 'monospace', color: '#64748b' }}>{member.id}</td>
                                <td style={{ padding: '1rem', fontWeight: '500', color: '#eff3c1' }}>{member.name}</td>
                                <td style={{ padding: '1rem' }}>{member.phone}</td>
                                <td style={{ padding: '1rem' }}>
                                    {member.cell !== 'None' ? (
                                        <span style={{ background: 'rgba(34, 193, 230, 0.1)', color: '#22c1e6', padding: '0.2rem 0.5rem', borderRadius: '4px', fontSize: '0.8rem' }}>{member.cell}</span>
                                    ) : (
                                ...
                                <td style={{ padding: '1rem' }}>
                                    <span style={{
                                        color: member.status === 'Active' ? '#4ade80' : (member.status === 'New' ? '#22c1e6' : '#94a3b8'),
                                        fontSize: '0.85rem'
                                    }}>
                                        ● {member.status}
                                    </span>
                                ...
                                <td style={{ padding: '1rem', textAlign: 'right' }}>
                                    <button style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: '#22c1e6', borderRadius: '0.3rem', padding: '0.3rem 0.6rem', fontSize: '0.8rem', cursor: 'pointer' }}>View</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            {/* Pagination */}
            <div style={{ padding: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid rgba(255,255,255,0.05)', color: '#94a3b8', fontSize: '0.85rem' }}>
                <div>Showing 1 to 5 of 2,450 entries</div>
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button style={{ background: 'rgba(255,255,255,0.05)', border: 'none', color: '#cbd5e1', padding: '0.3rem 0.6rem', borderRadius: '0.3rem', cursor: 'pointer' }}>Previous</button>
                    <button style={{ background: '#22c1e6', border: 'none', color: '#120D20', padding: '0.3rem 0.6rem', borderRadius: '0.3rem', cursor: 'pointer', fontWeight: 'bold' }}>1</button>
                    <button style={{ background: 'rgba(255,255,255,0.05)', border: 'none', color: '#cbd5e1', padding: '0.3rem 0.6rem', borderRadius: '0.3rem', cursor: 'pointer' }}>2</button>
                    <button style={{ background: 'rgba(255,255,255,0.05)', border: 'none', color: '#cbd5e1', padding: '0.3rem 0.6rem', borderRadius: '0.3rem', cursor: 'pointer' }}>Next</button>
                </div>
            </div>
        </div>
    );
